...
function getAll(cb){
	conn("select *, DATE_FORMAT(vehiculos.fecha_alta, '%d/%m/%Y') as fecha_alta_f, "+
		"DATE_FORMAT(vehiculos.chasis_fecha, '%d/%m/%Y') as chasis_fecha_f, "+
		"DATE_FORMAT(vehiculos.carro_fecha, '%d/%m/%Y') as carro_fecha_f "+
		"FROM vehiculos order by numero", cb);
}
